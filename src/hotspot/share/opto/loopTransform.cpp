      ...
      if (iff->outcnt() == 2) {         // Ignore dead tests
        ...
        if (bol && bol->req() > 1 && bol->in(1) &&
            ((bol->in(1)->Opcode() == Op_CompareAndExchangeB) ||
             ...
             (bol->in(1)->Opcode() == Op_XenandoahCompareAndExchangeP) ||
             (bol->in(1)->Opcode() == Op_XenandoahCompareAndExchangeN) ||
             (bol->in(1)->Opcode() == Op_XenandoahWeakCompareAndSwapP) ||
             (bol->in(1)->Opcode() == Op_XenandoahWeakCompareAndSwapN) ||
             (bol->in(1)->Opcode() == Op_XenandoahCompareAndSwapP) ||
             (bol->in(1)->Opcode() == Op_XenandoahCompareAndSwapN)))
